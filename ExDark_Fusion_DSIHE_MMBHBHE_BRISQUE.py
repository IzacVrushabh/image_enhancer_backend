...
folder = 'D:\Android_Projects\ExDarkImages\Bicycle'
...
for i in range(0, 4):
    ...
    for image in read_images:
        ...
        image = DSIHE_image + MMBEBHE_image
        img = PIL.Image.fromarray(image)
        Avg_Brisque.append(brisque.score(img))
        print(para_idx, idx)
        idx = idx + 1
    para_idx = para_idx + 1

    brisque_final_list.append("{:.3f}".format(np.array(Avg_Brisque).mean()))
print(brisque_final_list)
